...
def get_models(X, **kwargs):
    ...
    # tuned_parameters['xgb'] = {'regressor__n_estimators': [30, 100, 300]}

    # Random Forest
    rf = RandomForestRegressor()
    models['rf'] = Pipeline(steps=[('preprocessor', rf_preprocessor),
                                   ('regressor', rf)])
    tuned_parameters['rf'] = {'regressor__max_depth': [3, 5, 10, 15, 20]}
...
def main():
    ...
    fname_train = data_dir + "/Life_Expectancy/Life Expectancy Data"
    X_train = read_csv(fname_train)

    # print("X_train : ", X_train)
    # print("X_train shape = ", np.shape(X_train))

    X = X_train.dropna(axis=0, subset=['Life expectancy ']).head(300)  # keep only train data
    y = X['Life expectancy ']
    ...
    ax.set_title('Crossval Scores By Predictive Model With Tuning For 300 Samples (And LR Tuning)')
    ax.set_xticks(x)
